...
// 2025.11.12 handel subscription expiration always, as not all apps unsubscribe cleanly
//            this shall solve thread leaking and none working after a few days.
#if 1
    {
        ...
        while (sub_iter) {
            ...
            NPT_LOG_FINE_2("PLT_Service::NotifyChanged SID=%s, expiration=%lld ms",
                sub->GetSID().GetChars(),
                expiration.ToMillis() - now.ToMillis());

            if (expiration < now ) {
                NPT_LOG_FINE_1("PLT_Service::NotifyChanged expired SID=%s", sub->GetSID().GetChars());
